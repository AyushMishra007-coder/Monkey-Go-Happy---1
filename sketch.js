...
MonkeyStanding = loadImage("sprite_0.png");
  ...
  gameover=loadSound("GameOverSound.mp3");
}
...
function draw()
{
  ...
  if(gameState===START)
  {
   ...
   fill("red");
   textSize(20);
   text("Read all the instructions carefully before playing:-",30,80);
   fill("red");
   textSize(18);
   text("1.Press Space Key to Start the Game.",30,110);
   fill("black");
   text("2.Press UP Arrow Key for long jump.",30,135);
   text("3.Press Space Key to Jump.",30,160);
   text("4.Collect as many fruits as you can to increase your survival time.",30,190);
   text("5.Don't let the survival time to decrease to 0 or else the game will end.",30,220);
   text("6.Collect bananas to score and get survival time.",30,250);
   text("7.Avoid the obstacles otherwise you will lose 1 chance from 3.",30,280);
   text("8.Try to Score high, With more score game will get more difficult.",30,310);
   text("9.Avoid Long Jump unnecessary as it decreases the survival time.",30,340);
   fill("red");
   textSize(30);
...
function reset()
{
  ...
  gameOver.visible=false;
  restart.visible=false;
  //monkey.changeAnimation(MonkeyStanding);
}
